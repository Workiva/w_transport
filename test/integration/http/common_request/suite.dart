...
void runCommonRequestSuite(HttpIntegrationConfig config) {
  group('Common Request API', () {
    ...
    _runCommonRequestSuiteFor(config, 'StreamedRequest', ({bool withBody: false}) {
...
void _runCommonRequestSuiteFor(HttpIntegrationConfig config, String name, BaseRequest requestFactory({bool withBody})) {
  group(name, () {
    ...
    test('DELETE request with headers', () async {
      ...
      expect(json['headers'], containsPair('authorization', request.headers['authorization']));
      expect(json['headers'], containsPair('x-custom', request.headers['x-custom']));
      expect(json['headers'], containsPair('x-tokens', request.headers['x-tokens']));
    });
    ...
    test('GET request with headers', () async {
      ...
      expect(
          json['headers'], containsPair('x-tokens', request.headers['x-tokens']));
    });
