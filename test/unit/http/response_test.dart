...
void main() {
  ...
  group('StreamedResponse', () {
    ...
    test('content-length should be taken from headers', () {
      ...
      StreamedResponse response = new StreamedResponse.fromByteStream(200, 'OK', headers, new Stream.fromIterable([bytes]));
      expect(response.contentLength, equals(bytes.length));
    });

    test('body', () async {
      List<int> bytes = [1, 2, 3, 4];
      StreamedResponse response = new StreamedResponse.fromByteStream(200, 'OK', {}, new Stream.fromIterable([bytes]));
